...
static int count_rows (CSVData& d) {
    int i = 0;
    while (true) {
        auto r = d.get_row (i);
        if (r.empty ()) break;
...
TEST (CSVDataTest, ReadQuotedComma) {
    ...
    CSVData d;  // Ukloni csv:: ako nije u namespace-u
    d.read_file (fname);
...
TEST (CSVDataTest, AddSetDeleteColRow) {
    CSVData d;
    d.add_row (std::vector<std::string>{"x", "y", "z"});
    ...
    d.add_row (std::vector<std::string>{"keep"});
...
TEST (CSVDataTest, DeleteRowIfCallback) {
    CSVData d;
    d.add_row (std::vector<std::string>{"keep", "1"});
    d.add_row (std::vector<std::string>{"del", "2"});
    d.add_row (std::vector<std::string>{"del", "3"});
    EXPECT_EQ (count_rows (d), 3);

    auto cb = [] (int, const std::vector<std::string>& row, void*) -> bool {
...
TEST (CSVDataTest, SortByColumnNumericAndstring) {
    CSVData d;
    d.add_row (std::vector<std::string>{"2"});
    d.add_row (std::vector<std::string>{"10"});
    d.add_row (std::vector<std::string>{"1"});

    d.sort_by_col (0, CSVData::ASC);
    ...
    CSVData s;
    s.add_row (std::vector<std::string>{"b"});
    s.add_row (std::vector<std::string>{"a"});
    s.add_row (std::vector<std::string>{"c"});
    s.sort_by_col (0, CSVData::ASC);
...
TEST (CSVDataTest, MakeDataUnique) {
    CSVData d;
    d.add_row (std::vector<std::string>{"a", "1"});
    d.add_row (std::vector<std::string>{"b", "2"});
    d.add_row (std::vector<std::string>{"a", "1"});
    d.add_row (std::vector<std::string>{"b", "2"});
    d.add_row (std::vector<std::string>{"c", "3"});
...
TEST (CSVDataTest, WriteAndReadRoundTripWithQuotedField) {
    ...
    {
        CSVData d;
        d.add_row (std::vector<std::string>{"normal", "a,b,comma"});
        d.add_row (std::vector<std::string>{"plain", "value"});
        d.write_data (fname);
    }
