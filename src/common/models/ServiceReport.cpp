...
void ServiceReport::setId (int id) {
    if (!isValidId (id)) throw std::invalid_argument ("ID servisnog izvestaja mora biti izmedju 0 i 2147483646");
    this->id = id;
}
void ServiceReport::setWorkOrderID (int workOrderID) {
    if (!isValidWorkOrderID (workOrderID)) throw std::invalid_argument ("ID radnog naloga mora biti izmedju 0 i 2147483646");
