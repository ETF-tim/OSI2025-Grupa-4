...
CSVData::CSVData(CSVData &rhs) : m_is_modified(rhs.is_modified()), m_is_unified(rhs.is_unified()),
...
void CSVData::add_row(vector<string> row_data)
{
    ...
    if (new_row_size != m_cols && m_is_unified)
        m_is_unified = false;
    m_data.push_back(row_data);
    m_rows++;
    if (m_cols < new_row_size)
        m_cols = new_row_size;
}
                            ...
                            void *cbData)
{
    ...
    if (rows_to_delete.size() > 0)
    {
        ...
        for (int row = 0; row < rows_to_delete.size(); ++row)
            delete_row(rows_to_delete.at(row));
    }
}
...
void CSVData::delete_item(int row, int col)
{
    if (row >= m_rows || row < 0)
        return;
    if (col >= m_data.at(row).size() || col < 0)
        return;
...
void CSVData::_read_file(const std::string &filename, std::vector<std::vector<std::string>> &target, int &cols)
{
    ...
    while (os_safe_getline(input_file, line))
    {
        ...
        {
            if (DEBUG_MODE)
                cout << "Line " << current_line << ": empty line" << endl;
        }
    }
...
{
    for (int row = 0; row < m_data.size(); ++row)
        convert_date_format(old_format, new_format, row, column);
}
...
void CSVData::convert_date_format(const std::string &old_format, const std::string &new_format, int row, int column)
{
    ...
    std::tm od = {};
    std::istringstream iss(old_date);
    iss >> std::get_time(&od, old_format.c_str());
    if (iss.fail())
    {
        cerr << "Failed to parse date '" << old_date << "' with format '" << old_format << "'" << endl;
        return;
    }
